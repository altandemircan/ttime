...
/* helper: clear all 3D-only artifacts so 2D map can redraw clean */
function teardown3DOverlays() {
  const m = window._maplibre3DInstance;
  ...
  try {
    ...
    // remove flyroute segments
    const style = m.getStyle();
    if (style && Array.isArray(style.layers)) {
      style.layers
        .filter(l => l.id && l.id.startsWith('flyroute-line-'))
        .forEach(l => {
          if (m.getLayer(l.id)) m.removeLayer(l.id);
          if (l.source && m.getSource(l.source)) m.removeSource(l.source);
        });
    }

    // remove segment highlight
    if (m.getLayer('segment-highlight-layer')) m.removeLayer('segment-highlight-layer');
    if (m.getSource('segment-highlight-source')) m.removeSource('segment-highlight-source');

    // remove markers created for 3D
    (window._maplibreRouteMarkers || []).forEach(mm => { try { mm.remove(); } catch (_) {} });
    window._maplibreRouteMarkers = [];

    (window._segment3DMarkers || []).forEach(mm => { try { mm.remove(); } catch (_) {} });
    window._segment3DMarkers = [];

    if (window._hoverMarker3D) { try { window._hoverMarker3D.remove(); } catch (_) {} ; window._hoverMarker3D = null; }

  } catch (err) {
    console.warn('[3D teardown] error while cleaning 3D overlays', err);
  }
}