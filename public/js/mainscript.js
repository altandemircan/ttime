...
async function updateCart() {
    // 1. RESİM HAFIZASINI SIFIRLA (Her render'da başa dönmesi için)
    if (window.__usedCollageImages) window.__usedCollageImages.clear();
    else window.__usedCollageImages = new Set();
    ...
    // Cart Filtreleme (Boş objeleri temizle)
    ...
    if (!window.cart || window.cart.length === 0) {
        ...
        if (startBtn && typeof startWithMap === 'function') startBtn.onclick = startWithMap;
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        if (!dayContainer) {
            ...
            // Harita konteynerini koru (re-render için)
        ...
        const travelMode = typeof getTravelModeForDay === "function" ? getTravelModeForDay(day) : "car";
        ...
        for (let idx = 0; idx < dayItemsArr.length; idx++) {
            ...
            if (item.category === "Note") {
                  ...
                  <div class="cart-item">
                    <div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
                      ...
                      <div style="display:flex; align-items:center; gap:5px;">
                        <button class="remove-btn" onclick="removeFromCart(${currIdx})"><img src="img/remove-icon.svg" alt="Close"></button>
                ...
                if (window.categoryIcons && window.categoryIcons[item.category]) catIcon = window.categoryIcons[item.category];
                  ...
                  <div class="cart-item">
                    <div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
                      <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <div class="item-position">${listMarkerHtml}<img src="${item.image}" alt="${item.name}" class="cart-image"></div>
                    ...
                    <div class="content">
                      <div class="info-section">
                        ...
                        ${item.location ? `<div class="coords-info" style="margin-top:8px;">📍 Coords: ${Number(item.location.lat).toFixed(7)}, ${Number(item.location.lng).toFixed(7)}</div>` : ''}
                      ...
                      <button class="add-favorite-btn" data-name="${item.name}" data-category="${item.category}" data-lat="${item.location?.lat||''}" data-lon="${item.location?.lng||''}">
                        <span class="fav-heart"><img class="fav-icon" src="${(typeof isTripFav==='function' && isTripFav(item))?'/img/like_on.svg':'/img/like_off.svg'}" style="width:18px;height:18px;"></span>
                        <span class="fav-btn-text">${(typeof isTripFav==='function' && isTripFav(item))?"Remove":"Add to My Places"}</span>
                      </button>
                      ...
                      <div class="confirmation-container" id="confirmation-item-${currIdx}" style="display:none;">
                        <p>Are you sure you want to remove <strong>${item.name}</strong>?</p>
                        <div class="modal-actions">
            ...
            if (item.location && nextItem && nextItem.location) {
                let distanceStr = '', durationStr = '', prefix = '';
                const isInTurkey = typeof areAllPointsInTurkey === 'function' ? areAllPointsInTurkey([item.location, nextItem.location]) : false;

                if (!isInTurkey) {
                    const distM = typeof haversine === 'function' ? haversine(item.location.lat, item.location.lng, nextItem.location.lat, nextItem.location.lng) : 0;
                    ...
                    if (summary) {
                        distanceStr = summary.distance >= 1000 ? (summary.distance / 1000).toFixed(2) + " km" : Math.round(summary.distance) + " m";
                        durationStr = summary.duration >= 60 ? Math.round(summary.duration / 60) + " min" : Math.round(summary.duration) + " sec";
                    }
                }
                ...
                distanceSeparator.innerHTML = `<div class="separator-line"></div><div class="distance-label">${prefix}<span class="distance-value">${distanceStr}</span> · <span class="duration-value">${durationStr}</span></div><div class="separator-line"></div>`;
        ...
        if (anyDayHasRealItem && (!window.__hideAddCatBtnByDay || !window.__hideAddCatBtnByDay[day])) {
    ...
    addNewDayButton.onclick = function () { if(typeof addNewDay === 'function') addNewDay(this); };
    ...
    if (window.expandedMaps) Object.values(window.expandedMaps).forEach(({ expandedMap, day }) => { if (expandedMap) updateExpandedMap(expandedMap, day); });
    ...
    (function ensureSelectDatesButton() {
        ...
        btn.onclick = () => { if (typeof openCalendar === 'function') openCalendar(1); };
    })();

    // New Trip Plan (Cart içi)
    (function ensureNewChatInsideCart(){
        ...
        if (!newChat){
            ...
            newChat.onclick = function() {
                ...
                if (chatBox) {
                    ...
                    welcome.innerHTML = "<img src='img/avatar_aiio.png' class='profile-img'>Let's get started.";
        ...
        const cnt = window.cart.filter(i => i.name && !i._starter && !i._placeholder).length;
        newChat.style.display = cnt > 0 ? 'block' : 'none';
    })();
    ...
    (function ensurePdfButtonAndOrder() {
        ...
        if (datesBtn && pdfBtn) datesBtn.insertAdjacentElement('afterend', pdfBtn);
        else if (newChatBtn && pdfBtn && pdfBtn.parentNode !== cartRoot) cartRoot.insertBefore(pdfBtn, newChatBtn);
        if (pdfBtn && newChatBtn && document.body.contains(pdfBtn)) pdfBtn.insertAdjacentElement('afterend', newChatBtn);
    })();
    ...
    (function ensureTripDetailsBlock() {
        ...
        if (detailsBtn) {
            detailsBtn.onclick = () => { if (typeof showTripDetails === 'function') showTripDetails(window.cart.startDate); };
        }
    })();
