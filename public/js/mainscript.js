...
async function getCityFromCoordinates(lat, lng) {
    try {
        // Mevcut Geoapify yapını kullanarak reverse geocode yapıyoruz
        ...
        if (data.features && data.features.length > 0) {
            const props = data.features[0].properties;
            // Şehir ismini yakalamak için öncelik sırası (City -> Town -> County -> State)
...
async function handleMapAIClick(e) {
    const map = e.target; // Tıklanan harita
    const { lat, lng } = e.latlng;

    // A) Kullanıcıya hemen "Yükleniyor" popup'ı göster
    const popup = L.popup()
        .setLatLng([lat, lng])
        .setContent(`
            <div style="text-align:center; padding:10px; min-width:150px;">
                <div class="spinner" style="display:inline-block; width:12px; height:12px; border:2px solid #ccc; border-top-color:#333; border-radius:50%; animation:spin 1s linear infinite;"></div>
                <span style="margin-left:8px; font-weight:600; color:#555;">AI Analysing...</span>
            </div>
        `)
        .openOn(map);
    ...
    if (!locationData || !locationData.city) {
        popup.setContent('<div style="color:#d32f2f; padding:5px;">City info not found here.</div>');
        return;
    }

    // C) AI Servisine Sor (Senin llm-proxy.js endpoint'in)
    try {
        ...
        // D) Popup İçeriğini AI verisiyle güncelle
        const aiContent = `
            <div class="ai-map-popup" style="max-width:280px; font-family:'Satoshi', sans-serif;">
                <h3 style="margin:0 0 10px 0; padding-bottom:8px; border-bottom:1px solid #eee; display:flex; align-items:center; gap:6px; color:#1e293b;">
                    🤖 ${locationData.city}
                </h3>
                <div style="font-size:0.9rem; line-height:1.5; color:#475569;">
                    <p style="margin-bottom:8px;"><b>📝 Summary:</b> ${aiData.summary || 'No summary available.'}</p>
                    <p style="margin-bottom:8px;"><b>💡 Tip:</b> ${aiData.tip || 'No tip available.'}</p>
                    <p style="margin:0;"><b>✨ Highlight:</b> ${aiData.highlight || ''}</p>
                </div>
        ...
        console.error(err);
        popup.setContent('<div style="color:#d32f2f; padding:5px;">AI Service Unavailable. Try again.</div>');
    }
}
