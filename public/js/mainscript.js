...
async function renderRouteForDay(day) {
    ...
    const expandedMapDiv =
        document.getElementById(`expanded-map-${day}`) || document.getElementById(`expanded-route-map-day${day}`);
    if (expandedMapDiv) {
        let expandedScaleBar = document.getElementById(`expanded-route-scale-bar-day${day}`);
        if (!expandedScaleBar) {
            expandedScaleBar = document.createElement('div');
    ...
    // 5. FLYMODE— markerlar arasında düz hat
    if (points.length >= 2) {
        let totalKm = 0;
        let pairwiseSummaries = [];
        for (let i = 0; i < points.length; i++) {
            if (i > 0) {
                ...
                pairwiseSummaries.push({
                    ...
                    duration: Math.round((d / 4) * 3600)
                });
            }
        }
        let durationSec = Math.round(totalKm / 4 * 3600);
        const summary = {
            distance: Math.round(totalKm * 1000),
            duration: durationSec
        };
