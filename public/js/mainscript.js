...
if (
    ...
    (
        !(["car", "walk", "bicycle"].includes(travelMode)) ||
        (
            !(window.importedTrackByDay && window.importedTrackByDay[day] && window.routeLockByDay && window.routeLockByDay[day]) &&
            !routeData // eğer directions başarısızsa veya travelMode farklıysa
        )
    )
) {
    let totalKm = 0;
