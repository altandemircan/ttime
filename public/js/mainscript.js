...
for (let idx = 0; idx < dayItemsArr.length; idx++) {
  ...
  // FLY MODDA HER ZAMAN SEPARATOR GÖSTER
  const hasNextLoc =
    item.location &&
  ...
  const flySeparator = (travelMode === "fly" && hasNextLoc);
  const routeSeparator = (!flySeparator && hasNextLoc);

  if (flySeparator || routeSeparator) {
    let distanceStr = '';
    let durationStr = '';
    let autoGeneratedStr = '';

    if (flySeparator) {
      // YALNIZCA fly modda her zaman göster!
      const ptA = dayItemsArr[idx].location;
      const ptB = dayItemsArr[idx + 1].location;
      ...
      autoGeneratedStr = `<span class="auto-generated-label" style="font-size:11px;color:#888;">auto generated</span>`;
    } else {
      ...
      if (summary && typeof summary.distance === "number" && typeof summary.duration === "number") {
        ...
        autoGeneratedStr = ''; // Mutlaka boş olsun!
      } else {
        // fallback (haversine) car/bike/walk modunda -- yine boş!
        const ptA = dayItemsArr[idx].location;
        const ptB = dayItemsArr[idx + 1].location;
        const distM = haversine(ptA.lat, ptA.lng, ptB.lat, ptB.lng);
        const durSec = Math.round((distM / 1000) / 4 * 3600);
