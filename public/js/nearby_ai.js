...
function showCustomPopup(lat, lng, map, content, showCloseButton = true) {
    // 1. Önceki popup ve markerları temizle
    if (typeof closeNearbyPopup === 'function') closeNearbyPopup();
    
    // 2. Popup container oluştur ve ekle
    ...
    // 3. --- PULSE MARKER (MAVİ HALKA) EKLEME ---
    ...
    if (!document.getElementById('tt-pulse-styles')) {
        ...
        style.textContent = `
            ...
            .tt-pulse-dot {
                ...
                z-index: 10; animation: tt-pulse-dot 2s ease-in-out infinite;
            }
            .tt-pulse-ring {
                position: absolute; left: 50%; top: 50%;
            ...
            @keyframes tt-pulse-dot {
                0%, 100% { transform: translate(-50%, -50%) scale(1); box-shadow: 0 0 15px rgba(25, 118, 210, 0.8); }
                50% { transform: translate(-50%, -50%) scale(1.1); box-shadow: 0 0 25px rgba(25, 118, 210, 1); }
            }
    ...
    // Harita Tipine Göre Ekleme
    const isMapLibre = !!map.addSource;

    if (isMapLibre) {
        // --- 3D MOD (MapLibre) ---
        const el = document.createElement('div');
        el.className = 'tt-pulse-marker-container'; // Container class
        el.innerHTML = pulseHtml;
        
        // Önceki varsa temizle (Extra güvenlik)
        ...
        const pulseIcon = L.divIcon({
            html: pulseHtml,
            className: 'tt-pulse-marker-leaflet', // Leaflet için özel class
            iconSize: [40, 40],
            iconAnchor: [20, 20]
        });
