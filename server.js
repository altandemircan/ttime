...
app.post('/api/parse-trip', async (req, res) => {
    try {
        ...
        if (!userInput || typeof userInput !== 'string') {
            return res.status(400).json({ valid: false, error: 'userInput required' });
        }

        const groqKey = process.env.GROQ_API_KEY;
        if (!groqKey) {
            console.error('[parse-trip] GROQ_API_KEY missing');
...
Başka hiçbir açıklama, markdown işareti ya da ekstra metin ekleme, sadece JSON döndür.`;
        ...
        if (!groqRes.ok) {
            ...
            console.error('[parse-trip] Groq error:', errText);
            return res.status(502).json({ valid: false, error: 'ai_upstream_error' });
        }
        ...
        try {
            ...
            console.error('[parse-trip] JSON parse failed. Raw:', raw);
            return res.json({ valid: false });
        }

        if (!parsed.valid || !parsed.city || !parsed.days) {
            return res.json({ valid: false });
        }
        ...
        res.json({ valid: true, city: String(parsed.city).trim(), days });

    } catch (e) {
        console.error('[parse-trip] Error:', e);
